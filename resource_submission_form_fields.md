...
**Agent guidance:** Resolve the instrument against the controlled list at `/api/models/InstrumentObservatory/rows/all/`. Read the response's `data[]` array and **consider only SPASE-backed rows** — `identifier.startswith("https://spase-metadata.org/")`; the endpoint still contains ~63 legacy rows (blank or `helio.data.nasa.gov/...` identifiers) that must be ignored. Match by `type` 1 (instrument) and the canonical `name` with any parenthetical abbreviation stripped (e.g. `Magnetic Field Investigation`, not `Magnetic Field Investigation (MFI)`), preferring the `SMWG/...` namespace. **If more than one SPASE candidate remains after namespace/platform evidence, do not guess — record the bare name with no identifier and flag it for review.** Otherwise supply the canonical `name` + its SPASE `identifier` (the reliable de-duplication key — a free-typed or abbreviation-embedded name silently creates a duplicate). See the `submission-payload` / `update-payload` skills for the authoritative procedure.
...
**Agent guidance:** Resolve the observatory/mission against the controlled list at `/api/models/InstrumentObservatory/rows/all/`. Read the response's `data[]` array and **consider only SPASE-backed rows** — `identifier.startswith("https://spase-metadata.org/")`; ignore the ~63 legacy rows with blank or `helio.data.nasa.gov/...` identifiers. Match by `type` 2 (observatory) and the canonical `name` with any parenthetical abbreviation stripped (e.g. `Parker Solar Probe`, not `Parker Solar Probe (PSP)`), preferring the `SMWG/Observatory/...` namespace (note the canonical SMWG name may be the long form, e.g. SMWG/Observatory/THEMIS is "Time History of Events and Macroscale Interactions during Substorms"). **If more than one SPASE candidate remains after namespace/platform evidence, do not guess — record the bare name with no identifier and flag it for review.** Otherwise supply the canonical `name` + its SPASE `identifier` (the reliable de-duplication key). See the `submission-payload` / `update-payload` skills for the authoritative procedure.
